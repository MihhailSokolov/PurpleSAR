...
class ConfigHandler:
    ...
    def read_config(self, config_path: str) -> dict:
        yml_dict_default = YmlReader.load_file(
            os.path.join(
                os.path.dirname(__file__), "../configs/attack_range_default.yml"
            )
        )
        yml_dict = YmlReader.load_file(
        ...
        # Convert splunk_apps to comma-separated string
        if (
            "splunk_server" in yml_dict_default
            and "splunk_apps" in yml_dict_default["splunk_server"]
        ):
            yml_dict_default["splunk_server"]["splunk_apps"] = ",".join(
                yml_dict_default["splunk_server"]["splunk_apps"]
            )
        ...
        for parent_key in parent_keys_servers:
            if parent_key not in yml_dict:
                ...
                for windows_server in yml_dict[parent_key]:
                    yml_dict_default[parent_key].append(
                        yml_dict_default[parent_key + "_default"].copy()
                    )
    ...
    def validate_config(self, config: dict) -> None:
        if config["general"]["attack_range_password"] in [
            "ChangeMe123!",
            "Pl3ase-k1Ll-me:p",
        ]:
        ...
        for windows_server in config["windows_servers"]:
            if (
                windows_server["create_domain"] == "0"
                and windows_server["bad_blood"] == "1"
            ):
            ...
            if (i > 0) and windows_server["create_domain"] == "1":
                print(
                    "ERROR: create_domain=1 is only allowed for the first windows server in the list windows_servers."
                )
                sys.exit(1)
            i = i + 1
        ...
        if (
            ...
            print(
                "ERROR: Carbon Black Cloud or Crowdstrike Falcon can only used in AWS."
            )
            sys.exit(1)

        if (
            config["phantom_server"]["phantom_server"] == "1"
            and config["phantom_server"]["phantom_byo"] == "1"
        ):
            print(
                "ERROR: You can either create a phantom server or activate bring your own phantom but not both."
            )
            sys.exit(1)

        if config["splunk_server"]["byo_splunk"] == "1" and (
            config["phantom_server"]["phantom_byo"] == "1"
            or config["phantom_server"]["phantom_server"] == "1"
        ):
            print(
