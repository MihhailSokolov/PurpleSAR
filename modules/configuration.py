...
starting configuration for AT-ST mech walker
    ...
    if configuration["general"]["cloud_provider"] != "local":
        questions = [
            {  # reuse key pair?
                "type": "confirm",
                "message": "detected existing key in {0}, would you like to use it".format(
                    priv_key
                ),
                "name": "reuse_keys",
                "default": True,
        ...
        if "new_key_pair" in answers:
            if answers["new_key_pair"]:
                ...
                if configuration["general"]["cloud_provider"] == "aws":
                    ...
                    print(
                        "> new azure ssh pair created:\nprivate key: {0}\npublic key:{1}".format(
                            priv_key_path, pub_key_path
                        )
                    )
                else:
                    print(
                        "ERROR, we do not support generating a key pair for the selected provider: {}".format(
        ...
        questions = [
            ...
            },
            {
                ...
                "when": lambda answers: ("key_name" not in configuration["general"])
                and (configuration["general"]["cloud_provider"] == "azure"),
            },
            {
                # get region
                "type": "input",
        ...
        if "key_name" in answers:
            ...
            print(
                "> using ssh key name: {}".format(configuration["general"]["key_name"])
            )
    ...
    print(
        "> configuration file was written to: {0}, run `python attack_range.py build` to create a new attack_range\nyou can also edit this file to configure advance parameters".format(
